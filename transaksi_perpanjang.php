...
if($lambat > 3) {
	...
	$sql = $koneksi->query("UPDATE tb_transaksi SET tgl_kembali = '$hari_next' WHERE id_transaksi = $id_transaksi") or die(mysqli_error($koneksi));

	if($sql) {
		echo "<script>alert('Perpanjang peminjaman buku berhasil.');document.location.href = 'transaksi.php';</script>";
	} else {
		echo "<script>alert('Perpanjang peminjaman buku gagal.');document.location.href = 'transaksi.php';</script>";
	}
}
